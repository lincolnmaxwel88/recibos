...
export default function NovoInquilinoPage() {
  ...
  const [imovelPreSelecionado, setImovelPreSelecionado] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchImoveis = async () => {
      try {
        // Verificar se há um imóvel pré-selecionado na URL
        const urlParams = new URLSearchParams(window.location.search);
        const imovelId = urlParams.get('imovelId');
        if (imovelId) {
          setImovelPreSelecionado(imovelId);
          setFormData(prev => ({ ...prev, imovelId }));
        }
        ...
        setError('Falha ao carregar imóveis. Por favor, tente novamente.');
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Erro ao cadastrar inquilino');
      }

      // Redirecionar para a lista de inquilinos após o cadastro
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Novo Inquilino</h1>
        <Link 
          href="/inquilinos" 
      ...
      <div className="bg-gray-50 rounded-lg shadow-md p-6 border border-gray-200">
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-6 flex justify-end">
            ...
            >
              {submitting ? 'Salvando...' : 'Salvar Inquilino'}
            </button>
          </div>
        </form>
      </div>
